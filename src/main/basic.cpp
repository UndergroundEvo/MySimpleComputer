...
string
getVariableAddress (string variable)
{
  int varAddress = -1;
...
string
writeMathOperation (string variable1, int &count, ofstream &file, bool isLeft)
{
...
string
writeMathOperation (string variable1, string oper, string variable2,
                    int &count, ofstream &file, bool isLeft)
{
  string tmpName;
...
string
recursionMath (vector<string> mathArray, int &count, ofstream &file,
               bool isLeft)
{
  string nameTmp;
  if (mathArray.size () == 1)
    ...
    {
      nameTmp = writeMathOperation (mathArray[0], mathArray[1], mathArray[2],
                                    count, file, isLeft);
      return nameTmp;
    }
  ...
  if (mathArray[indexLastOper + 1][0] == '('
      && mathArray[mathArray.size () - 1][0] == ')')
    {
      tmp = 1;
...
string
writeMath (string str, int &count, ofstream &file)
{
...
int
findCache (string key)
{
  return rows.find (stoi (key))->second;
}

void
writeLogic (string str, int &count, ofstream &file, string address)
{
